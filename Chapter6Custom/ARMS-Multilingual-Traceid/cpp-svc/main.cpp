...
static std::map<std::string, std::string> LoadRootEnv()
{
  ...
  std::cerr << "[cpp] WARN: cannot open .env (tried /app/.env, ./.env, ../.env, ../../.env)\n";
  return env;
}
...
static void InitTracer(const std::map<std::string, std::string> &env)
{
  ...
  for (const auto &kv : ParseHeaders(rawHeaders))
  {
...
int main()
{
  ...
  svr.Get("/cpp/work", [&](const httplib::Request &req, httplib::Response &res) {
    ...
    parent_ctx = propagator->Extract(carrier, parent_ctx);
  ...
  std::cout << "[cpp] listening on 0.0.0.0:" << port << std::endl;
